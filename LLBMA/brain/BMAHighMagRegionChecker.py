...
class BMAHighMagRegionCheckerBatchedSerial:
    """
    # THIS IS THE SERIAL VERSION OF THE BMAHighMagRegionCheckerBatched CLASS FOR DEBUGGING PURPOSES
    A class representing a manager that crops and checks high magnification regions.
    --model_ckpt_path : the path to the checkpoint of the model
    --model : the model object
    """
    ...
    def check_high_mag_score(self, batch):
        focus_regions, image_tensor_stack = batch
        ...
        #     vol = VoL(focus_region.image)
        #     focus_region.VoL_high_mag = vol # TODO V5 Update, we will calculate the VoL in the dataloader

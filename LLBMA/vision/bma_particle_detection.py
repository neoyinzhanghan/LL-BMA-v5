...
def get_white_mask(image, verbose=False):
    ...
    if verbose:
        ...
        plt.figure()
        plt.title("Grayscale Intensity Histogram for White Mask Detection")
        plt.xlabel("Bins")
    ...
    if verbose:
        ...
        plt.figure()
        plt.title("Local Minima and Maxima in Intensity Histogram for White Mask")
        plt.xlabel("Bins")
        plt.ylabel("# of Pixels")
        plt.plot(bin_midpoints, histogram)
        plt.xlim([0, 256])
...
def get_background_mask(image, erosion_radius=35, median_blur_size=35, verbose=False):
    ...
    if verbose:
        # display the mask
        plt.figure()
        plt.title("Median Blurred White Mask")
        plt.imshow(mask, cmap="gray")
        plt.show()
    ...
    if verbose:
        # display the mask
        plt.figure()
        plt.title("Eroded Background Mask")
        plt.imshow(mask, cmap="gray")
        plt.show()
    ...
    if verbose:
        ...
        plt.figure()
        plt.title("Connected Components in Background Mask")
        plt.imshow(labels)
        plt.show()
...
def get_high_blue_signal_mask(
    ...
    if verbose:
        try:
            # display the image and pause the execution until the user presses a key
            cv2.imshow("Normalized Blue Channel", image)
            cv2.waitKey(0)
            # close all windows
            cv2.destroyAllWindows()
        except KeyboardInterrupt:
    ...
    if verbose:
        try:
            # display the image and pause the execution until the user presses a key
            cv2.imshow("Grayscale Blue Channel", image)
            cv2.waitKey(0)
            # close all windows
            cv2.destroyAllWindows()
        except KeyboardInterrupt:
    ...
    if verbose:
        try:
            # display the image and pause the execution until the user presses a key
            cv2.imshow("Median Blurred Blue Channel", image)
            cv2.waitKey(0)
            # close all windows
            cv2.destroyAllWindows()
        except KeyboardInterrupt:
    ...
    if verbose:
        try:
            # display the image and pause the execution until the user presses a key
            cv2.imshow("Dilated Blue Channel", image)
            cv2.waitKey(0)
            # close all windows
            cv2.destroyAllWindows()
        except KeyboardInterrupt:
    ...
    if verbose:
        try:
            # display the image and pause the execution until the user presses a key
            cv2.imshow("Thresholded Blue Channel Mask", image)
            cv2.waitKey(0)
            # close all windows
            cv2.destroyAllWindows()
        except KeyboardInterrupt:
...
def get_obstructor_mask(
    ...
    # Create a mask for the middle 90% of the image (the obstructor region)
    height, width = image.shape[:2]
    
    # Calculate the boundaries for the middle 90%
    x_start = int(width * 0.05)
    x_end = int(width * 0.95)
    y_start = int(height * 0.05)
    y_end = int(height * 0.95)
    
    # Create a black mask (zeros) with the same dimensions as the grayscale image
    mask = np.zeros((height, width), dtype="uint8")
    
    # Set the middle 90% rectangle to white (255)
    # We're keeping the mask as-is, so this directly defines the obstructor region as white
    mask[y_start:y_end, x_start:x_end] = 255
    
    if verbose:
        # display the mask
        plt.figure()
        plt.title("Rectangular Obstructor Mask")
        plt.imshow(mask, cmap="gray")
        plt.show()
    
    # No inversion - the obstructor region is white (255) and the outer area is black (0)
    ...
    if verbose:
        # Display the histogram
        plt.figure()
        plt.title("Grayscale Intensity Histogram for Obstructor Detection")
        plt.xlabel("Bins")
        plt.ylabel("# of Pixels")
        plt.plot(bin_midpoints, histogram)
        plt.xlim([0, 256])
    ...
    if verbose:
        ...
        # make the line corresponding to first_min_after_first_max(local_minima, local_maxima) longer than the rest
        plt.figure()
        plt.title("Local Minima and Maxima in Intensity Histogram for Obstructor Detection")
        plt.xlabel("Bins")
        plt.ylabel("# of Pixels")
        plt.plot(bin_midpoints, histogram)
        plt.xlim([0, 256])
    ...
    if verbose:
        # display the mask
        plt.figure()
        plt.title("Initial Obstructor Mask")
        plt.imshow(mask, cmap="gray")
        plt.show()
    ...
    if verbose:
        # display the mask
        plt.figure()
        plt.title("Median Blurred Obstructor Mask")
        plt.imshow(mask, cmap="gray")
        plt.show()
    ...
    if verbose:
        # display the mask
        plt.figure()
        plt.title("Final Eroded Obstructor Mask")
        plt.imshow(mask, cmap="gray")
        plt.show()
    """
...
def marrow_boxing(
    ...
    if verbose:
        ...
        plt.figure()
        plt.subplot(1, 2, 1)
        plt.title("Original Particle Detection Mask")
        plt.imshow(mask, cmap="gray")
        plt.subplot(1, 2, 2)
        plt.title("Boxed Particle Detection Mask")
    ...
    if verbose:
        plt.figure()
        plt.subplot(1, 3, 1)
        plt.title("Original BMA Image")
        plt.imshow(image)
        plt.subplot(1, 3, 2)
        plt.title("Particle Detection Mask")
        plt.imshow(mask, cmap="gray")
        plt.subplot(1, 3, 3)
        plt.title("Particle Detection Overlay on Original Image")
...
if __name__ == "__main__":
    ...
    # display the mask
    plt.figure()
    plt.title("Final Particle Detection Mask")
    plt.imshow(mask, cmap="gray")
    plt.show()
